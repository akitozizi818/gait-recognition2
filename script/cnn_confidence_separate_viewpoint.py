...
for dataset_name in dataset_names:
    # output_dir = f"../data/out_results_scale_confidence_0.4/{dataset_name}"
    output_dir = f"../data/test/{dataset_name}"
    ...
    for i in range(config["iterations"]):
        ...
        if test_acc > best_test_acc:
            ...
            history_dict = {
                ...
                for key, values in best_history.history.items()
            }
    ...
    # JSONファイルに保存
    with open(os.path.join(output_dir, "all_predictions_file_name.json"), "w", encoding="utf-8") as file:
